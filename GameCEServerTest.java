...
public class GameCEServerTest implements Serializable {
            ...
            while (turnsMade < maxTurns) {
                ...
                for (int i = 0; i < playerServer.length; ) {
                    ...
                    if (null == playerPockerStatus) {
                        ...
                        playerPockerStatus=testModifyHandPoker(playerPockerStatus);
                    ...
                    if(new String(playerPockerStatus.getPokerTop().getNumber()+playerPockerStatus.getPokerTop().getType()).equals(temp)){
                        ...
                        if(playerPockerStatus.getPokerTop().getNumber().equals(CommonNumVar.K.getNumber())){
                            for (int j = 0; j < playerServer.length; j++) {
                                playerServer[j].sendTurnNo(-2);
                                playerServer[j].sendObject("Reverses the direction of play!--NOT WORK");
                            }
                            if(shun)
                                shun=true;
                            else
...
//                            for (int j = 0; j < playerServer.length; j++) {
//                                playerServer[j].sendTurnNo(-2);
//                                playerServer[j].sendObject("The next player "+(i+1)+" will miss their turn!");
//                            }
//                            if(shun){
...
//                        }
    ...
    private PokerStatus testModifyHandPoker(PokerStatus playerPockerStatus) {

        System.out.println("Please input the discard pile top poker: eg HK");
        String line=scannerUtil();
        if(!"0".equals(line)) {
            Poker poker = new Poker();
            poker.setSimpleType(line.substring(0, 1));
            poker.setNumber(line.substring(1));
            playerPockerStatus.setPokerTop(poker);
        }
        System.out.println("Please input the hand poker: eg HK,DQ");
        String newline=scannerUtil();
        if(!"0".equals(newline)) {
            String newlines[] = newline.split(",");
            List<Poker> handPoker = new ArrayList<>();
            for (int i = 0; i < newlines.length; i++) {
                Poker p = new Poker();
                p.setSimpleType(newlines[i].substring(0, 1));
                p.setNumber(newlines[i].substring(1));
                p.setScore(RmPoker.getScore(newlines[i].substring(1)));
                handPoker.add(p);
            }
            PokerHand pokerHand = new PokerHand();
            pokerHand.setPokers(handPoker);
            playerPockerStatus.setPokerHand(pokerHand);
        }
        System.out.println("Please input the dui poker: eg HK,DQ");
        newline=scannerUtil();
        if(!"0".equals(newline)) {
            String newlines1[] = newline.split(",");
            PokerDui pokerDui = new PokerDui();
            for (int i = 0; i < newlines1.length; i++) {
                Poker p = new Poker();
                p.setSimpleType(newlines1[i].substring(0, 1));
                p.setNumber(newlines1[i].substring(1));
                p.setScore(RmPoker.getScore(newlines1[i].substring(1)));
                pokerDui.add(p);
            }
            playerPockerStatus.setPokerDui(pokerDui);
        }
        System.out.println("ok!");
        return playerPockerStatus;
    }

    private String scannerUtil() {
        String s=null;
        boolean f=true;
        while(f){
            try {
    ...
    public static void main(String args[]) throws Exception {
        GameCEServerTest sr = new GameCEServerTest();
